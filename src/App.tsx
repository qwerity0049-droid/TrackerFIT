...
import { Activity, LayoutDashboard, History, User, PlusCircle } from 'lucide-react';
...
function App() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 font-sans flex flex-col md:flex-row">
      {/* Sidebar / Bottom Nav */}
      <nav className="bg-white border-r border-gray-200 md:w-64 flex-shrink-0 fixed md:relative bottom-0 w-full md:h-screen z-10">
        ...
        <div className="flex md:flex-col justify-around md:justify-start p-2 md:p-4 space-y-0 md:space-y-2">
          <NavItem icon={<LayoutDashboard />} label="Дашборд" active={activeTab === 'dashboard'} onClick={() => setActiveTab('dashboard')} />
          <NavItem icon={<PlusCircle />} label="Новая тренировка" active={activeTab === 'log'} onClick={() => setActiveTab('log')} />
      ...
      <main className="flex-1 p-4 md:p-8 pb-24 md:pb-8 overflow-y-auto">
        <div className="max-w-4xl mx-auto">
          ...
          {activeTab === 'log' && <WorkoutLogger onSave={addWorkout} />}
...
function NavItem({ icon, label, active, onClick }: { icon: React.ReactNode, label: string, active: boolean, onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className={`flex flex-col md:flex-row items-center md:space-x-3 p-2 md:p-3 rounded-xl transition-colors ${
    ...
    >
      ...
      <span className={`text-xs md:text-sm font-medium mt-1 md:mt-0 ${active ? 'text-indigo-700' : ''}`}>{label}</span>
    </button>
  );
}
